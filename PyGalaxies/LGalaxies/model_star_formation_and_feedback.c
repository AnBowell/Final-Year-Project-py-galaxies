...
double compute_SN_reheat(double stars, double ColdGas, double Vvir)
{
  ...
  // Guo2010 - eq 18 & 19
      reheated_mass = modelParams.feedback_reheating_epsilon * stars * (.5+1./pow(Vvir/modelParams.reaheat_pre_vel,modelParams.reaheat_slope));
      
//       printf(" Vvir = %0.5e and Vconst = %0.5e" ,Vvir);
      
//       
//  	  if (reheated_mass * Vvir * Vvir > stars * (modelParams.EtaSN * modelParams.EnergySN))
//  	      reheated_mass = stars * (modelParams.EtaSN * modelParams.EnergySN) / (Vvir * Vvir);
    }
  else{
    reheated_mass=0.;
    }
...
double update_stars_due_to_reheat(double mass_cold_gas, double *stars, double Vc)
{
  double reheated_mass, frac;

  reheated_mass = compute_SN_reheat(*stars, mass_cold_gas, Vc);
  
  if((*stars + reheated_mass) > mass_cold_gas)
    {
      frac = mass_cold_gas / (*stars + reheated_mass);
      *stars *= frac;
    }
                   ...
                   double time, double dt, int nstep)
{
  ...
  double tdyn, strdot=0., stars, cold_crit,reheated_mass;
  




  //3.086e+19 --> to km so tdyn is in seconds.
  tdyn = (rad_cold_gas * 3.086e+19) / Vc; 
  // https://academic.oup.com/mnras/article/413/1/101/1060133
  cold_crit = modelParams.SfrColdCrit * Vc/200. * rad_cold_gas*100; //h factor;

  //standard star formation law (Croton2006, Delucia2007, Guo2010, Henriques2015)
  if(mass_cold_gas > cold_crit)
	strdot = modelParams.SfrEfficiency * (mass_cold_gas - cold_crit) / tdyn;
  else
	strdot = 0.0;
  ...
  // 3.154e+7 converts years to seconds.
  stars = (strdot * dt * 3.154e+7);

  // ~~~~~~~~~~~~~ Now add some feedback ~~~~~~~~~~~~~~~~~~//

  if (stars > 0.)
   	reheated_mass = update_stars_due_to_reheat(mass_cold_gas, &stars, Vc);

//   if (stars > 0.)
//     SN_feedback(p, centralgal, stars, "ColdGas");


  if (stars > mass_cold_gas)
  	  stars = mass_cold_gas;

  struct ReturnTuple to_return = {stars, reheated_mass};
  
//   ReturnTuple.Returnstars = stars;
//   ReturnTuple.Return_reheated_mass = reheated_mass;
